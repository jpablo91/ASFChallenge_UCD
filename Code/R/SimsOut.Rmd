...
# New pig herd Cases 
# NOTE: Changed a little bit the code, I think it would be easier to visualize if we only color in the scale the ones that had at least one case, and fill with red (or could be other color) the ones where the initial pig herd outbreaks happened
Hx %>%
  left_join(Hx_sim03, by = 'idhex') %>%
  mutate(Epidemic = ifelse(is.na(cases), Epidemic, 0)/NSims,
         index_case = ifelse(is.na(cases), NA, 1),
         Epidemic = ifelse(Epidemic == 0, NA, Epidemic)) %>%
  ggplot() +
  geom_sf(aes(fill = Epidemic)) +
  geom_sf(data = subset(Hx, ph_cass > 0), fill = "red4")
```
...
# Hx_sim03 %>%
#   filter(Epidemic != 0) %>%
#   mutate(pEpidemic = Epidemic/NSims) %>%
#   arrange(desc(pEpidemic))
```
