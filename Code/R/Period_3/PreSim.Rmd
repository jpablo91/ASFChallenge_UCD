...
-  How likely the epidemic is to fade in the coming four months (day 230)?
-  Are there any risks to consider for long term?
  -  Second wave
  -  Endemicity in wild boar  
  -  Others?
-  Suggestions to limit the risk
-  Should control measures be lifted ofter 230 days?
  ...
  mutate(Farms = ifelse(is.na(Farms), 0.0001, Farms), # If there is no PH pop, set a very small number for the model
    density_s = scales::rescale(x = density, to = c(0.2,0.6)), # Score based on the density
     WB_score = scales::rescale(x = Forest, to = c(0,.5)) + # score based on the forst cover and EB density
          ...
          outdoor = ifelse(is.na(is_outdoor  ), 0, is_outdoor  ),
    outdoor = scales::rescale(outdoor, c(0.01, 0.5))) %>%
  write_sf(here(paste0("Data/Period_3/out/Hx_", HxRes, ".shp")))
```
