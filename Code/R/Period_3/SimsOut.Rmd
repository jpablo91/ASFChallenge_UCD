...
## all the cases are current outbreaks (index_case variable), so there is an error mapping this
m00 = Hx %>%
  left_join(Hx_sim00_summary, by = 'idhex') %>% 
  # filter(!is.na(Pop)) %>%
  mutate(Epidemic = ifelse(is.na(cases), Epidemic, 0)/nsims,
    index_case = ifelse(is.na(cases), NA, 1)) %>% filter(idhex == 2643)
  PlotMap
...
Hx_sim00 %>%
  filter(cycle == 279, idhex == 2643, Sim == 1)
  # filter(Disease_status == 'Epidemic')
  # count(infection_source)
```
