...
# Convert to a field
Border <- as(raster::extent(Is), "SpatialPolygonssdsds") %>%
  sf::st_as_sf()
...
new_hex_grid = function(cellsize, border, orig_shp) {
  ...
  hexgrid = st_join(HexPols_sf, orig_shp) %>%
    filter(!is.na(ID)) %>%
...
```{r}
herds50 <- read.csv(here("Datass/Period_1/herds_day_50.csv"))
######## Summarize herds ########
herds50 <- herds50 %>%
  mutate(N=1) %>%
...
```{r}
Mov <- read.csv(here("Data/Period_1/moves_Players_day_50.csv"))
...
MovHx %>%
  ...
  # slice(1:200) %>%
  write.csv(., here("Data/Period_1/out/MovHx.csv"), row.names = FALSE, quote = FALSE)
```
...
Hx %>%
  ...
  write_sf(here("Data/Period_1/out/Hx.shp"))
```
