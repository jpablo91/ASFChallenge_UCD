...
MovHx %>%
  ...
  select(ID, sourceHx, destHx) %>%
  # slice(1:200) %>%
  write.csv(., "../Data/Period_1/out/MovHx.csv", row.names = F, quote = F)

MovHx %>%
  filter(sourceHx == 1429)
```
...
Vals <- lapply(1:nrow(Hx), function(x){
  mean(raster::extract(WBr, Hx[x,])[[1]], na.rm = T)
})
...
Hx %>%
  filter(!is.na(Pop)) %>%
  mutate(density_s = scales::rescale(x = density, to = c(0.1,0.6)),
         WB_score = scales::rescale(x = Forest, to = c(0,.5)) +
...
```{r}
HxDF %>%
  arrange(desc(Result)) # wild boar cases seem to hapen in forest areas
```
